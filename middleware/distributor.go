...
func getModelRequest(c *gin.Context) (*ModelRequest, bool, error) {
	...
	if strings.HasPrefix(c.Request.URL.Path, "/v1/image_generation") {
		modelRequest.Model = common.GetStringIfEmpty(modelRequest.Model, "image-01")
...
// getTaskOriginModelName 从已有任务中回填模型，避免模型限制校验误判。
...
func SetupContextForSelectedChannel(c *gin.Context, channel *model.Channel, modelName string) *types.NewAPIError {
	...
	case constant.ChannelTypeXunfei:
		c.Set("api_version", channel.Other)
	case constant.ChannelTypeGemini, constant.ChannelTypeGeminiInteractions:
		c.Set("api_version", channel.Other)
