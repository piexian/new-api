...
func TestGenerateOAuthCodeScopesRegistrationCredential(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "unmarked credential clears stale credential",
			target:   "/api/oauth/state?aff=stale-code",
			existing: "stale-code",
			expected: nil,
		},
		{
			name:     "blank registration credential clears stale credential",
			target:   "/api/oauth/state?registration=true&aff=%20%20",
			existing: "stale-code",
			expected: nil,
		},
		{
			name:     "marked registration stores normalized credential",
			target:   "/api/oauth/state?registration=true&aff=%20fresh-code%20",
			existing: "stale-code",
			expected: "fresh-code",
		},
	}
