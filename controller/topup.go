...
func GetTopUpInfo(c *gin.Context) {
	...
	data := gin.H{
		...
		"subscription_purchase_max_quantity": model.GetSubscriptionPurchaseMaxQuantity(),
...
func EpayNotify(c *gin.Context) {
	...
	if verifyInfo.TradeStatus == epay.StatusTradeSuccess {
		...
		if topUp.Status == common.TopUpStatusPending {
			...
			completedTopUp, quotaToAdd, completed, err := model.CompleteEpayTopUp(verifyInfo.ServiceTradeNo, verifyInfo.Type, c.ClientIP())
			if err != nil {
			...
			model.RecordTopupLog(topUp.UserId, fmt.Sprintf("使用在线充值成功，充值金额: %v，支付金额：%f", logger.LogQuota(quotaToAdd), topUp.Money), c.ClientIP(), topUp.PaymentMethod, "epay", c.GetHeader("User-Agent"))
		}
	} else {
...
func AdminCompleteTopUp(c *gin.Context) {
	...
	if err := model.ManualCompleteTopUp(req.TradeNo, c.ClientIP(), c.GetHeader("User-Agent")); err != nil {
		common.ApiError(c, err)
		return
	}
