...
type SubscriptionResetResult struct {
	...
	AffectedSubscriptions []SubscriptionResetTarget `json:"-"`
}
...
func NormalizeSubscriptionPurchaseMode(mode string) string {
	...
	default:
		return SubscriptionPurchaseModeConcurrent
	}
}
...
func hasActiveSubscriptionForUpgradeGroupTx(tx *gorm.DB, userId int, upgradeGroup string, now int64, excludeId int) (bool, error) {
	upgradeGroup = strings.TrimSpace(upgradeGroup)
	if upgradeGroup == "" {
		return false, nil
	}
	query := tx.Model(&UserSubscription{}).
		Where("user_id = ? AND status = ? AND end_time > ? AND upgrade_group = ?", userId, "active", now, upgradeGroup)
	if excludeId > 0 {
...
func getSubscriptionPrevUserGroupForCreateTx(tx *gorm.DB, userId int, currentGroup, upgradeGroup string) (string, error) {
	...
	res := tx.Where("user_id = ? AND status = ? AND upgrade_group = ? AND prev_user_group <> '' AND prev_user_group <> ?",
		userId, "active", upgradeGroup, upgradeGroup).
...
func getSubscriptionStartTimeForCreateTx(tx *gorm.DB, userId int, planId int, now int64, purchaseMode string) (time.Time, error) {
	start := time.Unix(now, 0)
	if NormalizeSubscriptionPurchaseMode(purchaseMode) != SubscriptionPurchaseModeRenew || planId <= 0 {
		return start, nil
	}
	var lastSub UserSubscription
	res := tx.Where("user_id = ? AND plan_id = ? AND status = ? AND end_time > ?",
	...
	if res.RowsAffected > 0 && lastSub.EndTime > now {
		return time.Unix(lastSub.EndTime, 0), nil
	}
	return start, nil
}
...
func downgradeUserGroupForSubscriptionTx(tx *gorm.DB, sub *UserSubscription, now int64) (string, error) {
	...
	hasActiveSameGroup, err := hasActiveSubscriptionForUpgradeGroupTx(tx, sub.UserId, upgradeGroup, now, sub.Id)
	if err != nil {
		return "", err
	}
...
func CreateUserSubscriptionFromPlanTx(tx *gorm.DB, userId int, plan *SubscriptionPlan, source string) (*UserSubscription, error) {
	return CreateUserSubscriptionFromPlanWithModeTx(tx, userId, plan, source, SubscriptionPurchaseModeConcurrent)
}

func CreateUserSubscriptionFromPlanWithModeTx(tx *gorm.DB, userId int, plan *SubscriptionPlan, source string, purchaseMode string) (*UserSubscription, error) {
	if tx == nil {
	...
	start, err := getSubscriptionStartTimeForCreateTx(tx, userId, plan.Id, nowUnix, purchaseMode)
	if err != nil {
		return nil, err
	}
	...
	prevGroup, err := getSubscriptionPrevUserGroupForCreateTx(tx, userId, currentGroup, upgradeGroup)
	if err != nil {
		return nil, err
	}
	if upgradeGroup != "" {
...
func AdminBindSubscription(userId int, planId int, purchaseMode string, sourceNote string) (string, error) {
	if userId <= 0 || planId <= 0 {
	...
	var completedEvent *SubscriptionCompletedEvent
	err = DB.Transaction(func(tx *gorm.DB) error {
		sub, err := CreateUserSubscriptionFromPlanWithModeTx(tx, userId, plan, "admin", purchaseMode)
		if err == nil {
			completedEvent = &SubscriptionCompletedEvent{
				UserId:             userId,
				SubscriptionId:     sub.Id,
...
func PurchaseSubscriptionWithBalance(userId int, planId int) error {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		completedEvent = &SubscriptionCompletedEvent{
			...
			NextResetTime:      sub.NextResetTime,
			ResetPeriod:        NormalizeResetPeriod(plan.QuotaResetPeriod),
			PaymentAmount:      plan.PriceAmount,
			PaymentMethod:      PaymentMethodBalance,
			PaymentProvider:    PaymentProviderBalance,
			SubscriptionSource: sub.Source,
		}
...
func buildSubscriptionResetResult(plan *SubscriptionPlan, subs []UserSubscription, advanceResetTime bool) *SubscriptionResetResult {
	...
	targets := make([]SubscriptionResetTarget, 0, len(subs))
	seenUsers := make(map[int]struct{}, len(subs))
	for _, sub := range subs {
		targets = append(targets, SubscriptionResetTarget{UserId: sub.UserId, SubscriptionId: sub.Id})
	...
	return &SubscriptionResetResult{
		...
		AffectedSubscriptions: targets,
	}
}

func resetSubscriptionNotificationLocks(targets []SubscriptionResetTarget) {
	for _, target := range targets {
		common.ResetQuotaNotificationSendLocks(target.UserId, "subscription", target.SubscriptionId)
	}
}
...
func AdminResetUserSubscriptionsByPlan(userId int, planId int, advanceResetTime bool) (*SubscriptionResetResult, error) {
	...
	resetSubscriptionNotificationLocks(result.AffectedSubscriptions)
	return result, nil
}
...
func ExpireDueSubscriptions(limit int) (int, error) {
	expired, err := ExpireDueSubscriptionsWithDetails(limit)
	return len(expired), err
}

// ExpireDueSubscriptionsWithDetails returns only subscriptions transitioned by this call.
func ExpireDueSubscriptionsWithDetails(limit int) ([]ExpiredSubscriptionInfo, error) {
	if limit <= 0 {
		limit = 200
	}
	now := GetDBTimestamp()
	...
	expired := make([]ExpiredSubscriptionInfo, 0, len(subs))
	...
	for userId := range userIds {
		cacheGroup := ""
		var userExpired []ExpiredSubscriptionInfo
		err := DB.Transaction(func(tx *gorm.DB) error {
			var dueSubs []UserSubscription
			if err := lockForUpdate(tx).Where("user_id = ? AND status = ? AND end_time > 0 AND end_time <= ?", userId, "active", now).
				Order("end_time desc, id desc").
			...
			for _, dueSub := range dueSubs {
				planTitle := fmt.Sprintf("Plan #%d", dueSub.PlanId)
				if plan, planErr := getSubscriptionPlanByIdTx(tx, dueSub.PlanId); planErr == nil && strings.TrimSpace(plan.Title) != "" {
					planTitle = plan.Title
				}
				userExpired = append(userExpired, ExpiredSubscriptionInfo{
					UserId:              dueSub.UserId,
					SubscriptionId:      dueSub.Id,
					PlanId:              dueSub.PlanId,
					PlanTitle:           planTitle,
					ExpiredAt:           dueSub.EndTime,
					SubscriptionSource:  dueSub.Source,
					AllowWalletOverflow: dueSub.AllowWalletOverflow,
				})
				target, err := downgradeUserGroupForSubscriptionTx(tx, &dueSub, now)
				if err != nil {
					return err
				}
				if target != "" {
		...
		expired = append(expired, userExpired...)
		if cacheGroup != "" {
			_ = UpdateUserGroupCache(userId, cacheGroup)
		}
	}
...
func fillSubscriptionPreConsumeResultTx(tx *gorm.DB, result *SubscriptionPreConsumeResult, record *SubscriptionPreConsumeRecord) error {
	...
	result.QuotaResetPeriod = SubscriptionResetNever
	if sub.PlanId > 0 {
		plan, err := getSubscriptionPlanByIdTx(tx, sub.PlanId)
		if err != nil {
			return err
		}
		result.QuotaResetPeriod = NormalizeResetPeriod(plan.QuotaResetPeriod)
	}
	return nil
}
...
func preConsumeUserSubscription(requestId string, userId int, modelName string, requestGroup string, enforceRequestGroup bool, quotaType int, amount int64) (*SubscriptionPreConsumeResult, error) {
	...
	err := DB.Transaction(func(tx *gorm.DB) error {
		...
		for _, candidate := range candidates {
			...
			if plan != nil {
				reset, err := maybeResetUserSubscriptionWithPlanTx(tx, &sub, plan, now)
				if err != nil {
					return err
				}
				if reset {
					resetTargets = append(resetTargets, SubscriptionResetTarget{UserId: sub.UserId, SubscriptionId: sub.Id})
				}
...
func ResetDueSubscriptions(limit int) (int, error) {
	...
	for _, sub := range subs {
		...
		err = DB.Transaction(func(tx *gorm.DB) error {
			...
			var resetErr error
			reset, resetErr = maybeResetUserSubscriptionWithPlanTx(tx, &locked, plan, now)
			if resetErr != nil {
				return resetErr
			}
			if reset {
		...
		if reset {
			common.ResetQuotaNotificationSendLocks(subCopy.UserId, "subscription", subCopy.Id)
		}
	}
