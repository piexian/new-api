...
func formatUserLogs(logs []*Log, startIdx int, logLanguage ...string) {
	for i := range logs {
		logs[i].ChannelName = ""
	...
	if len(logLanguage) > 0 {
		LocalizeLogs(logs, logLanguage[0])
	}
}

func GetLogByTokenId(tokenId int, logLanguage ...string) (logs []*Log, err error) {
	order := "id desc"
	if common.UsingLogDatabase(common.DatabaseTypeClickHouse) {
		order = clickHouseLogOrder("")
	}
	err = LOG_DB.Model(&Log{}).Where("token_id = ?", tokenId).Order(order).Limit(common.MaxRecentItems).Find(&logs).Error
	formatUserLogs(logs, 0, logLanguage...)
	return logs, err
}
...
func RecordChannelManageLog(channelId int, content string, adminInfo map[string]interface{}) {
	log := &Log{
		UserId:    0,
		Username:  "system",
		CreatedAt: common.GetTimestamp(),
	...
	if err := LOG_DB.Create(log).Error; err != nil {
		common.SysLog("failed to record channel manage log: " + err.Error())
	}
}
...
func GetUserLogs(userId int, logType int, startTimestamp int64, endTimestamp int64, modelName string, tokenName string, startIdx int, num int, group string, requestId string, upstreamRequestId string, userAgent string, ip string, logLanguage ...string) (logs []*Log, total int64, err error) {
	...
	formatUserLogs(logs, startIdx, logLanguage...)
	return logs, total, err
}
