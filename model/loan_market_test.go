...
func TestBlacklistLiftsWhenPerpetualFundingsFullyRepaid(t *testing.T) {
	...
	require.NoError(t, DB.Transaction(func(tx *gorm.DB) error {
		...
		require.NoError(t, err)
		info, _, _, err := distributeRepayment(tx, acc, fundings, 100_000, now, "manual", false)
		require.NoError(t, err)
...
func TestBlacklistNotLiftedRightAfterWriteoff(t *testing.T) {
	...
	require.NoError(t, DB.Transaction(func(tx *gorm.DB) error {
		...
		info, _, _, err := distributeRepayment(tx, acc, fundings, 105_000, now, "manual", false)
		require.NoError(t, err)
		require.NotNil(t, info)
...
func TestWrittenOffFundingExcludedFromRepayment(t *testing.T) {
	...
	require.NoError(t, DB.Transaction(func(tx *gorm.DB) error {
		...
		info, allocs, _, err := distributeRepayment(tx, acc, fundings, 105_000, now, "manual", false)
		require.NoError(t, err)
		require.NotNil(t, info)
