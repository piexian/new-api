...
type Token struct {
	...
	RateLimit          int            `json:"rate_limit" gorm:"default:0"`    // 令牌整体每分钟请求数上限，0=不限制
	IpRateLimit        int            `json:"ip_rate_limit" gorm:"default:0"` // 单个来源IP每分钟请求数上限，0=不限制
...
func (token *Token) Update() (err error) {
	...
	err = DB.Model(token).Select("name", "status", "expired_time", "remain_quota", "unlimited_quota",
		...
		"rate_limit", "ip_rate_limit").Updates(token).Error
	return err
}
